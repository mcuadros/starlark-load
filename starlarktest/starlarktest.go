...
//go:generate esc -o assert.star.go -pkg starlarktest assert.star
...
func LoadAssertModule() (starlark.StringDict, error) {
	once.Do(func() {
		...
		filename := "assert.star"
		src := FSMustString(false, "/"+filename)

		thread := new(starlark.Thread)
		assert, assertErr = starlark.ExecFile(thread, filename, src, predeclared)
	})
